...
#Heatmap(scaledmat, name = "Normalized Count", column_title = "VG6 WT Samples Naive v. Treated", column_names_gp = gpar(fontsize = 6), row_title = "IL1a Pathway Genes", row_names_gp = gpar(fontsize = 10), row_order = order(as.numeric(gsub("row", "", rownames(scaledmat)))), column_order = order(as.numeric(gsub("column", "", colnames(scaledmat)))))
Heatmap(scaledmat, name = "Normalized Count", column_title = "VG6 WT Samples Naive v. Treated", column_names_gp = gpar(fontsize = 6), row_title = "1L1a Pathway Genes", row_names_gp = gpar(fontsize = 10))
