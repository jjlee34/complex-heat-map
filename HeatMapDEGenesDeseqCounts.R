...
df1=read_excel('/Users/leejorj/Downloads/Selected DE genes Naive 2&3 vs. C.mast 1&3.xlsx')
a=df1[,1]
...
write.table(scaledmat, file=paste(outputpath = "/Users/leejorj/Downloads/","scaledmat.txt",sep=""), quote=F, row.names = TRUE, sep = "\t")
#Heatmap(scaledmat, name = "Normalized Count", column_title = "VG6 WT Samples Naive v. Treated", column_names_gp = gpar(fontsize = 6), row_title = "Selected DE Genes", row_names_gp = gpar(fontsize = 8), row_order = order(as.numeric(gsub("row", "", rownames(scaledmat)))), column_order = order(as.numeric(gsub("column", "", colnames(scaledmat)))))
Heatmap(scaledmat, name = "Normalized Count", column_title = "VG6 WT Samples Naive v. Treated", column_names_gp = gpar(fontsize = 6), row_title = "Selected DE Genes", row_names_gp = gpar(fontsize = 6))
print(nrow(scaledmat))
